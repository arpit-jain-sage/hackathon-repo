...
create_pull_request() {
  ...
  # 2.2>&1: redirects stderr to stdout, so errors (if any) also goes to logs.txt file.
};

list_pull_request() {
  print_divider double_line;
  echo "Below are the list of open PRs authored by you:-"
